...
`architectures/arch_generator.py` automatically generates a target device model. To create a target device model, a custom file, with the same structure as the latter, should be created. The python file requires the same libraries as `arch_generator.py` and should be developed in the same folder. Alternatively, `arch_generator.py` itself can be modified. There are several methods available for the generation of custom arrays, which include (but are not limited to):
